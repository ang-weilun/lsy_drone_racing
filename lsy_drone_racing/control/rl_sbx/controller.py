"""Deployment shim for the SBX-trained policy.
...
class RLSBXController(Controller):
    ...
    config : dict
        Race configuration. Requires ``controller.checkpoint`` (path to a
        run-dir or a single ``step_*`` dir) and the usual ``sim.physics`` /
        ``sim.drone_model`` entries.
    """
...
def _config_value(config: Any, *keys: str) -> Any:
    ...
    *keys : str
    ...
    -------
    Any
        The value at ``config[keys[0]][keys[1]]...``.
