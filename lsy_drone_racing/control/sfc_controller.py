...
class StateController(Controller):
    ...
    def compute_control(self, obs, info=None):
        ...
        if t >= self.planner.t_total and self.planner.target_gate_idx >= len(self.planner.gates_pos):
            self._finished = True

        des_pos, des_vel, des_acc = self.planner.evaluate(t)
        yaw = float(np.arctan2(des_vel[1], des_vel[0])) if np.linalg.norm(des_vel[:2]) > 0.1 else 0.0
    ...
    def _record_replan_snapshot(self, obs):
    ...
    def render_callback(self, sim) -> None:
        ...
        draw_points(sim, self.planner.des_pos_spline(u).reshape(1, -1), rgba=(1.0, 0.0, 0.0, 1.0), size=0.04)
        draw_line(sim, self.planner.des_pos_spline(np.linspace(0.0, 1.0, 100)), rgba=(0.0, 1.0, 0.0, 1.0))
