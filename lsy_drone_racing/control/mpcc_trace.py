...
Set ``MPCC_TRACE_DIR`` to an output directory to enable; each episode writes
one compressed ``.npz`` with per-tick telemetry, per-replan geometry, and
episode metadata, consumed by ``scripts/trace_autopsy.py``. When unset, no
recorder is constructed, so flight code pays only a single ``is None`` check
per tick. Sim-only: ``scripts/deploy.py`` never calls ``episode_callback``.
"""
