...
def create_acados_model(parameters: dict, config: MPCCConfig) -> AcadosModel:
    ...
    model.u = U_aug
    model.p = p
    # EXTERNAL cost = Gauss-Newton LS part 0.5*(y-yref)^T W (y-yref) plus a true
    # LINEAR progress reward -mu_progress*v_theta (classic MPCC); a least-squares
    # cost alone can't express the linear term. v_theta has zero quadratic weight
    # (see _cost_weight_vectors).
...
class AttitudeMPC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        # A/B hook: MPCC_PMM_ENABLED=1/true toggles the PMM reference generator
        # without editing config files (policed by the online code-check).
        pmm_env = os.environ.get("MPCC_PMM_ENABLED")
        if pmm_env is not None:
            self.planner_config.pmm_enabled = pmm_env.strip().lower() in ("1", "true")
    ...
    def _update_spline(self) -> None:
        ...
        # Gate-pinned curvature smoothing: spreads each gate's near-cusp turn over a
        # longer arc so peak curvature drops to a feasible lateral-accel value without
        # a global speed cut. FITPACK weighted spline pins the gate crossings, relaxes
        # the rest to ref_smooth_tol RMS. Default off.
        if self.mpcc_config.ref_smooth and len(pts) > 6 and s[-1] > 1e-6:
            keep = np.concatenate(([True], np.diff(s) > 1e-9))
            pts_k, s_k = pts[keep], s[keep]
            weights = np.ones(len(s_k))
            for gate in self.planner.gates_pos:
                gate_s = s_k[int(np.argmin(np.linalg.norm(pts_k - gate, axis=1)))]
                weights[np.abs(s_k - gate_s) < self.mpcc_config.ref_gate_pin_dist] = 50.0
            smooth_target = len(s_k) * self.mpcc_config.ref_smooth_tol**2
            tck, _ = splprep(pts_k.T, w=weights, u=s_k / s_k[-1], s=smooth_target, k=3)
            pts = np.asarray(splev(u_samples, tck)).T
        ...
        # Coarse curvature profile kappa(s) for the curvature-coupled v_theta cap.
        # Finite-differenced on a ~0.08 m resample so sub-cm spline cusps (PMM-fit
        # artifacts) average out and only macroscopic curvature survives.
        n_coarse = max(8, int(self._s_total / 0.08) + 1)
        s_coarse = np.linspace(0.0, self._s_total, n_coarse)
        p_coarse = self._des_pos_spline(s_coarse)
        d1 = np.gradient(p_coarse, s_coarse, axis=0)
        d2 = np.gradient(d1, s_coarse, axis=0)
        self._kappa_s = s_coarse
        self._kappa_vals = np.linalg.norm(np.cross(d1, d2), axis=1) / (
            np.linalg.norm(d1, axis=1) ** 3 + 1e-9
        )

        # Arc-length progress theta of each gate center, consumed by the tunnel pinch.
        # Brute-force projection, recomputed only on replan.
    ...
    def _record_replan_trace(self) -> None:
        ...
        # Planner's own spline (the magenta control-point curve) sampled over its
        # native u in [0, 1], to compare against the MPCC's resampled CubicSpline
        # (self._des_pos_spline) and isolate any distortion introduced by _update_spline.
        u_dense = np.linspace(0.0, 1.0, mpcc_trace.SPLINE_SAMPLES)
        planner_spline = np.asarray(self.planner.des_pos_spline(u_dense), dtype=np.float32)
    ...
    def compute_control(
        ...
        if replanned:
            self._update_spline()
            # Re-anchor progress via a full-arc-length closest-point search rather than
            # resetting theta to 0: when the planner HOLDS its previous plan (PMM
            # infeasible) the body is already far along the spline, and a narrow
            # forward-window search mis-anchors theta, decoupling progress from the
            # body and crashing the horizon into the ground (seed44).
            s_full = np.linspace(0.0, self._s_total, 300)
            ref_dists = np.linalg.norm(self._des_pos_spline(s_full) - obs["pos"], axis=1)
            self._current_theta = float(s_full[int(np.argmin(ref_dists))])
        ...
        # Curvature-coupled v_theta cap: where the reference curves tightly, lower the
        # per-stage v_theta upper bound so a_lat = v_theta**2 * kappa stays within the
        # budget (the solver otherwise tracks the infeasible high-curvature reference at
        # MAX_V_THETA into the frame/ground). Default off; straights keep MAX_V_THETA.
        apply_v_cap = self.mpcc_config.v_theta_curv_cap or self.mpcc_config.v_theta_gate_ease
        if apply_v_cap:
            theta_nodes = np.clip(
                self._current_theta + self._shooting_nodes * self._current_v_theta,
                0.0,
                self._s_total,
            )
            v_cap = np.full_like(theta_nodes, self.mpcc_config.MAX_V_THETA)
            if self.mpcc_config.v_theta_curv_cap:
                kap_nodes = np.interp(theta_nodes, self._kappa_s, self._kappa_vals)
                v_curv = np.sqrt(self.mpcc_config.v_theta_a_lat / np.maximum(kap_nodes, 1e-6))
                v_curv = np.clip(
                    v_curv, self.mpcc_config.v_theta_curv_floor, self.mpcc_config.MAX_V_THETA
                )
                v_cap = np.minimum(v_cap, v_curv)
            if self.mpcc_config.v_theta_gate_ease and len(self._theta_gates):
                # Ease v_theta toward v_theta_gate_min near the nearest UPCOMING gate
                # (same upcoming-gate arc distance the tunnel pinch uses), so the body is
                # not driven through the oblique crossing faster than it can track.
                target_gate_idx = getattr(self.planner, "target_gate_idx", -1)
                upcoming = (
                    self._theta_gates[target_gate_idx:]
                    if 0 <= target_gate_idx < len(self._theta_gates)
                    else self._theta_gates
                )
                d_gate = np.min(
                    np.abs(theta_nodes[:, None] - np.asarray(upcoming)[None, :]), axis=1
                )
                ease = np.exp(-(d_gate**2) / (2 * self.mpcc_config.v_theta_gate_sigma**2))
                v_gate = (
                    self.mpcc_config.MAX_V_THETA
                    - (self.mpcc_config.MAX_V_THETA - self.mpcc_config.v_theta_gate_min) * ease
                )
                v_cap = np.minimum(v_cap, v_gate)
            base_ubx = self._ocp.constraints.ubx
            for j in range(1, self._N):
                ubx_j = base_ubx.copy()
                ubx_j[4] = float(v_cap[j])  # v_theta slot (idxbx position 4)
        ...
        num_iters = self.mpcc_config.sqp_iters_per_step
        if replanned or param_diff > 0.5:
            num_iters = max(num_iters, self.mpcc_config.NLP_SOLVER_MAX_ITER)
    ...
    def render_callback(self, sim: object) -> None:
        ...
        if hasattr(self.planner, "capsules") and self.planner.capsules is not None:
            ...
            for cap in self.planner.capsules:
                # Render-only: optionally suppress the red gate-frame keep-out
                # capsules (toggled by the render tool); flight behavior is
                # unaffected since this lives in the debug-draw callback.
                if cap.is_gate and getattr(self, "_hide_gate_capsules", False):
                    continue
                rgba_phys = (
